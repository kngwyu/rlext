...
class LogStore:
    ...
    def __init__(self) -> None:
        self.inner: t.DefaultDict[str, t.List[t.Any]] = defaultdict(list)
    ...
    def __getitem__(self, index: t.Union[int, slice]) -> "LogStore":
        res: t.DefaultDict[str, t.List[t.Any]] = defaultdict(list)
...
class SummaryConfig(t.NamedTuple):
    indices: t.List[str]
    interval: int
    color: str
    describe_range: bool
...
class ExperimentLogger:
    ...
    def __init__(self, show_summary: bool = True) -> None:
        ...
        self._store: t.DefaultDict[str, LogStore] = defaultdict(LogStore)
        self._summary_config: t.DefaultDict[str, SummaryConfig] = defaultdict(
            lambda: SummaryConfig(["sec"], 1, "black", True)
        )
    ...
    def setup_from_script_path(
        ...
        fingerprint: t.Optional[t.Dict[str, str]] = None,
    ) -> None:
    ...
    def retrive(self, logdir: Path) -> t.Tuple[int, int]:
        self.logdir = logdir
    ...
    def setup_logdir(self, fingerprint: t.Dict[str, str] = {}) -> None:
    ...
    def submit(self, name: str, **kwargs) -> None:
        ...
        if self._show_summary:
            interval = self._summary_config[name].interval
    ...
    def summary_config(
        ...
        if "sec" not in indices:
            indices.append("sec")
        self._summary_config[name] = SummaryConfig(
            indices, interval, color, describe_range
        )

    def show_summary(self, name: str) -> None:
        indices, interval, color, describe_range = self._summary_config[name]
